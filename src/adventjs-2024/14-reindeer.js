...
Nota: Ten en cuenta que el array de establos siempre tendrá el mismo tamaño que el array de renos y 
que siempre los establos serán únicos.
*/

function minMovesToStables(reindeer, stables) {
  const raideer = reindeer.sort((a, b) => a - b);
  const stable = stables.sort((a, b) => a - b);
  let moves = 0;
  for (let i = 0; i < raideer.length; i++) {
    moves += Math.abs(raideer[i] - stable[i]);
  }
  return moves;
}
